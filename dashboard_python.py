...
with tab6:
    ...
    if archivo_cm is not None:
        ...
        # FILTRO GLOBAL SOLO POR MES
        df_cm = df_cm[df_cm["MES"].isin(meses_sel)]

        st.success(
            f"Archivo cargado correctamente: {len(df_cm)} registros tras filtro"
        )

        st.dataframe(df_cm.head(), use_container_width=True)
