...
# -------------------------------
# LEER ARCHIVO DE CORRECTIVOS
# -------------------------------
ruta_correctivos = BASE_DIR / "correctivos.xlsx"

if ruta_correctivos.exists():
    df_correctivos = pd.read_excel(ruta_correctivos, engine="openpyxl")
    df_correctivos.columns = df_correctivos.columns.str.strip().str.upper()

    # Convertir fechas
    df_correctivos["FECHA INICIO"] = pd.to_datetime(
        df_correctivos["FECHA INICIO"],
        errors="coerce"
    )

    # Calcular días abiertos
    hoy = pd.Timestamp.today().normalize()
    df_correctivos["DIAS ABIERTOS"] = (
        hoy - df_correctivos["FECHA INICIO"]
    ).dt.days

    # Filtrar correctivos abiertos
...
with tab5:
    ...
    if df_correctivos_abiertos.empty:
        st.info("No hay correctivos abiertos.")
    else:
        st.metric(
            "Total de correctivos abiertos",
            len(df_correctivos_abiertos)
        )
        ...
        columnas_existentes = [
            ...
            if col in df_correctivos_abiertos.columns
        ]

        st.dataframe(
            df_correctivos_abiertos[columnas_existentes],
            use_container_width=True,
            hide_index=True,
            column_config={
                "NUMERO OT": st.column_config.TextColumn("Numero de OT"),
                "DESCRIPCION": st.column_config.TextColumn("Descripcion"),
                "ESPECIFICACIONES": st.column_config.TextColumn("Refacción Solicitada"),
                "CANTIDAD": st.column_config.NumberColumn("Cantidad", format="%d"),
                "RESPONSABLE": st.column_config.TextColumn("Responsable"),
                "FECHA INICIO": st.column_config.DateColumn("Fecha Inicio"),
                "DIAS_ABIERTOS": st.column_config.NumberColumn(
                    "Dias Abiertos", format="%d"
                ),
