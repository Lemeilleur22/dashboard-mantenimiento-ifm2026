...
st.write(
    df[df["MES"].astype(str).str.upper() == "MAYO"]
    [["MES", "TIEMPO_REPARACION_(MIN)"]]
)
st.write("COLUMNAS:")
st.write(df.columns.tolist())

st.write("ULTIMAS 20 FILAS:")
st.write(df.tail(20))

st.write("CONTEO POR MES:")
st.write(df["MES"].value_counts(dropna=False))
...
with tab6:
    ...
    if archivo_cm is not None:
        ...
        supabase.table("historico_correctivos").delete().neq(
            "wonum", "").execute()
        supabase.table("historico_correctivos").insert(records).execute()
