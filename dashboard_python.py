...
with tab6:
    ...
    if archivo_cm is not None:
        ...
        st.write("Ejemplo fecha limpia:", df_upload["cdate"].iloc[0])
        ...
        st.write("Fechas parseadas CSV:", df_upload["cdate"].notna().sum())
    ...
    response_hist = supabase.table(
        "historico_correctivos").select("*").execute()
    ...
    if not df_cm.empty:
        ...
        df_cm["MES"] = df_cm["cdate"].dt.month.map(meses_num)

        # solo aplicar filtro si hay fecha
        if "TODOS" not in meses_sel:
            df_cm = df_cm[
                df_cm["MES"].isin(meses_sel) |
                df_cm["MES"].isna()
            ]

        df_cm["status"] = df_cm["status"].fillna("").astype(str)

        abiertas = len(df_cm[df_cm["status"].str.upper() != "CLOSE"])
        cerradas = len(df_cm[df_cm["status"].str.upper() == "CLOSE"])
        total_cm = len(df_cm)
        ...
        st.write("Fechas válidas:", df_cm["cdate"].notna().sum())

        df_tend = df_cm[df_cm["cdate"].notna()].copy()
