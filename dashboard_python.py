    ...
    if es_admin:
        ...
        if archivo is not None:
            ...
            if st.button("🔄️ Sincronizar con sistema"):
                ...
                # SETS
                # --------------
                set_db = set(df_db["NUMERO_CM"].dropna().unique())
                set_csv = set(df_carga["numero_cm"].dropna(
                ).unique()) if not df_db.empty else set()

                # ------------
                # LOGICA
                # -----------
                cm_nuevos = set_csv - set_db
                cm_a_cerrar = set_db - set_csv

                # -------------
                # NUEVOS INSERT
                # -------------
                nuevos = df_carga[df_carga["numero_cm"].isin(cm_nuevos)]
