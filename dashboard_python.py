...
with tab6:
    ...
    if archivo_cm is not None:
        ...
        # convertir TODO a python nativo
        for col in df_upload.columns:
            df_upload[col] = df_upload[col].apply(
                lambda x: None if pd.isna(x) else str(x)
            )

        # mantener fecha como None real
        df_upload["cdate"] = df_upload["cdate"].replace("None", None)
        ...
        # limpieza extrema JSON-safe
        records = json.loads(json.dumps(records, default=str))

        st.write(records[:2])   # debug
        ...
        st.success(f"{len(df_upload)} registros guardados")
        st.rerun()
