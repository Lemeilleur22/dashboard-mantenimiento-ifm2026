...
with tab5:
    ...
    if es_admin:
        ...
        if archivo is not None:
            ...
            if st.button("🔄️ Sincronizar con sistema"):
                ...
                if not df_db.empty:
                    ...
                    df_db["NUMERO_CM"] = df_db["NUMERO_CM"].astype(str)

                # --------------
                # SETS
                # --------------
                set_csv = set(df_carga["numero_cm"])
                set_db = set(df_db["NUMERO_CM"]) if not df_db.empty else set()

                # -------------
                # NUEVOS INSERT
                # -------------
                nuevos = df_carga[~df_carga["numero_cm"].isin(set_db)]

                # -----------------------------
                # CERRAR LOS QUE YA NO EXISTEN
                # -----------------------------
                a_cerrar = df_db[
                    (~df_db["NUMERO_CM"].isin(set_csv)) &
                    (df_db["ESTATUS"] != "REALIZADO")
                ]

                hoy = datetime.now().date().isoformat()

                # Cerrar en Supabase
                for _, row in a_cerrar.iterrows():
                    supabase.table("correctivos").update({
                        "estatus": "REALIZADO",
                        "fecha_cierre":  hoy
                    }).eq("id", int(row["ID"])).execute()
                ...
                st.success(f"""
                           ✅ Nuevos: {len(nuevos)}
                           ❌ Cerrados automaticamente: {len(a_cerrar)}
                """)
            ...
            # Convertir a lista
            existentes_set = set([x["numero_cm"] for x in existentes.data])

            # Filtrar solo nuevos
            df_nuevos = df_carga[~df_carga["numero_cm"].isin(existentes_set)]

            st.write(f"Nuevos registros a insertar: {len(df_nuevos)}")

            # -----------------------------
            # PASO 3: SUBIR A SUPABASE
            # -----------------------------
            if st.button("Subir a sistema"):
                if len(df_nuevos) == 0:
                    st.warning("No hay nuevos registros para subir.")
                else:
                    datos = df_nuevos.to_dict(orient="records")

                    supabase.table("correctivos").insert(datos).execute()

                    st.success(
                        f"{len(df_nuevos)} correctivos cargados correctamente.")
                    st.rerun()
