...
if ruta_correctivos.exists():
    ...
    df_correctivos.columns = (
        df_correctivos.columns.str.strip().str.upper()
    )
    df_correctivos = df_correctivos.rename(columns={
        "NUMERO CM" : "NUMERO_CM",
        "DESCRIPCIÓN DE CORRECTIVO" : "REFFACION_SOLICITADA",
    ...
    if "DIAS_ABIERTOS" in df_correctivos.columns:
        df_correctivos["DIAS_ABIERTOS"] = pd.to_numeric(
            df_correctivos["DIAS_ABIERTOS"], errors="coerce"
        )
    
    #Filtrar correctivos abiertos
    if "ESTATUS" in df_correctivos.columns:
        df_correctivos_abiertos = df_correctivos[
            df_correctivos["ESTATUS"].astype(str).str.upper() != "REALIZADO"
        ].copy()
    else:
        df_correctivos_abiertos = df_correctivos.copy()
    ...
    if not df_correctivos_abiertos.empty:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "DIAS_ABIERTOS" : st.column_config.NumberColumn("Dias Abierto", format="%d"),
            }
        )
    else:
