    ...
    if es_admin:
        ...
        if archivo is not None:
            ...
            if st.button("🔄️ Sincronizar con sistema") and not st.session_state.sync_done:
                ...
                # -----------------------
                existentes = supabase.table(
                    "correctivos").select("*").execute()
                df_db = pd.DataFrame(existentes.data)

                if not df_db.empty:
                    df_db.colums = df_db.columns.str.strip().str.lower()

                    if "numero_cm" not in df_db.columns:
                        for col in df_db.columns:
                            if "numero" in col and "cm" in col:
                                df_db = df_db.rename(
                                    columns={col: "numero_cm"})
                ...
                # MERGE INTELIGENTE
                # -------------------
                st.write("Columnas DB:", df_db.columns.tolist())
                st.write("Columnas CSV:", df_carga.columns.tolist())

                df_merge = df_carga.merge(
                    df_db,
                    on="numero_cm",
                    how="left",
                    suffixes=("_csv", "_db")
                ...
                def hay_cambio(row):
                    if pd.isna(row.get("refaccion_solicitada_db")):
                        return "NUEVO"

                    if (
                        str(row["refaccion_solicitada_csv"]) != str(
                            row.get("refaccion_solicitada_db"))
                        or int(row["cantidad_csv"]) != int(row.get("cantidad_db", 0))
                    ):
                        return "ACTUALIZAR"

                    return "IGUAL"

                df_merge["ACCION"] = df_merge.apply(hay_cambio, axis=1)

                # -----------------
                # FILTRAR
                # -----------------
                df_nuevos = df_merge[df_merge["ACCION"] == "NUEVO"]
                df_updates = df_merge[df_merge["ACCION"] == "ACTUALIZAR"]

                # -----------------------
                # INSERT NUEVOS
                # -----------------------
                if not df_nuevos.empty:
                    supabase.table("correctivos").insert(
                        df_nuevos[[
                            "numero_cm",
                            "refaccion_solicitada_csv",
                            "cantidad_csv",
                            "fecha_inicio_csv"
                        ]].rename(columns={
                            "refacciom_solicitada_csv": "refaccion_solicitada",
                            "cantidad_csv": "cantidad",
                            "fecha_inicio_csv": "fecha_inicio"
                ...
                for _, row in df_updates.iterrows():
                    supabase.table("correctivos").update({
                        "refaccion_solicitada": row["refaccion_solicitada_csv"],
                        "cantidad": int(row["cantidad_csv"]),
                    }).eq("numero_cm", row["numero_cm"]).execute()
                ...
                🔄️ Actualizados: {len(df_updates)}
                ⚠️ Sin cambios: {len(df_merge['ACCION'] == 'IGUAL').sum()}""")

                st.success(f"""
                    ✅ Registros procesados: {len(df_carga)}
