...
with tab6:
    ...
    if archivo_cm is not None:
        ...
        df_cm = pd.read_csv(archivo_cm, encoding="latin1")
        st.write("Filas cargadas:", len(df_cm))
        st.write(df_cm.columns.tolist())
        ...
        # LIMPIEZA FECHAS
        # -------------------------
        df_cm["cdate"] = df_cm["cdate"].astype(str)

        df_cm["cdate"] = df_cm["cdate"].apply(
            lambda x: re.sub(r"[^A-Za-z0-9,: ]", " ", x)
        )

        df_cm["cdate"] = df_cm["cdate"].str.replace(
            r"\s+",
            " ",
            regex=True
        ).str.strip()

        df_cm["cdate"] = pd.to_datetime(
            df_cm["cdate"],
            errors="coerce"
        )
        ...
        df_cm["MES"] = df_cm["cdate"].dt.month.map(meses_map)

        # SOLO FILTRO DE MES GLOBAL
        ...
        # -------------------------
        top_ubicaciones = (
            df_cm.get("ldesc", pd.Series(["SIN UBICACION"] * len(df_cm)))
        ...
        tendencia["ORDEN"] = tendencia["MES"].map(orden_meses)
        tendencia = tendencia.sort_values("ORDEN")
